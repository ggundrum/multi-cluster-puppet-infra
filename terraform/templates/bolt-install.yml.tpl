#cloud-config
package_update: true
package_upgrade: true

#cloud-config
# Add groups to the system
# Adds the ubuntu group with members 'root' and 'sys'
# and the empty group hashicorp.
groups:
  - ubuntu: [root,sys]
  - hashicorp

# Add users to the system. Users are added after groups are added.
users:
  - default
  - name: terraform
    gecos: terraform
    shell: /bin/bash
    primary_group: hashicorp
    sudo: ALL=(ALL) NOPASSWD:ALL
    groups: users, admin
    lock_passwd: false
  - name: ubuntu
    gecos: ubuntu
    shell: /bin/bash
    primary_group: ubuntu
    sudo: ALL=(ALL) NOPASSWD:ALL
    groups: users,admin
    ssh_authorized_keys:
      - ssh-rsa 

runcmd:
 - wget https://apt.puppet.com/puppet-tools-release-bionic.deb
 - sudo dpkg -i puppet-tools-release-bionic.deb
 - sudo apt-get update
 - sudo apt-get install puppet-bolt
 - ln -s /opt/puppetlabs/bin/bolt /home/ubuntu/bolt
 - su - ubuntu
 - cd /home/ubuntu
 - bolt project init large_ha_peadm --modules puppetlabs-peadm | tee -a /home/ubuntu/cmd.txt
 - ls -alh /home/ubuntu/.ssh | tee -a checkforkey.txt
 - ls -alh /home/ubuntu | tee -a checkforparamsjson.txt
 - whoami > whoami.txt
 - cd /home/ubuntu
 - bolt plan run peadm::install --params @params.json | tee -a /home/ubuntu/install.txt
 
 